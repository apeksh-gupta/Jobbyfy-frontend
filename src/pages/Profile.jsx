...
export default function Profile() {
  ...
  const handleGoogleDriveUpload = () => {
    ...
    // Note: If using Create React App, use process.env.REACT_APP_...
    const clientId = import.meta.env.VITE_GOOGLE_DRIVE_CLIENT_ID;
    const apiKey = import.meta.env.VITE_GOOGLE_DRIVE_API_KEY;

    if (!clientId || !apiKey) {
      alert("Configuration Error: Google Drive API keys are missing in .env file");
      return;
    }
    ...
    openPicker({
      ...
      callbackFunction: (data) => {
        if (data.action === "picked") {
          ...
          // 2. Update profile with the selected file URL
          handleChange("resumeUrl", file.url);
        }
      },
    });
  };
  ...
  // ------------------------------
  return (
    <div className="min-h-screen bg-linear-to-br from-[#f7faff] to-[#eef2ff] px-4 py-10">
      <div className="max-w-4xl mx-auto">
        {/* HEADER */}
        <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3 mb-8">
          <UserCircleIcon className="w-10 h-10 text-blue-600" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-3">
            ...
            <div className="col-span-1 md:col-span-2 mt-2">
              ...
              <p className="text-xs text-gray-500 mt-2">
                * Note: Please ensure the file is shared as <strong>"Anyone with the link"</strong> so our AI can read it.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white rounded-xl border shadow-sm p-6 mb-10">
          ...
          {profile.education.map((edu, i) => (
            <div
              key={i}
              className="p-5 mb-6 border rounded-xl bg-gray-50 shadow-sm"
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <Input
                    ...
                    className={`${
                      errors.education[i]?.schoolName ? "border-red-500" : ""
                    }`}
                  />
                ...
                <Input
                  label="Major"
                  value={edu.major}
                  onChange={(e) => updateEduField(i, "major", e.target.value)}
                />
                ...
                <Input
                  ...
                  value={edu.startDate?.slice(0, 10)}
                  onChange={(e) =>
                    updateEduField(i, "startDate", e.target.value)
                  }
                />
                ...
                <Input
                  ...
                  value={edu.percentageOrCGPA}
                  onChange={(e) =>
                    updateEduField(i, "percentageOrCGPA", e.target.value)
                  }
                />
              </div>
          ...
          {profile.experience.map((exp, i) => (
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <Input
                    ...
                    className={`${
                      errors.experience[i]?.companyName ? "border-red-500" : ""
                    }`}
                  />
                ...
                <Input
                  label="Role"
                  value={exp.role}
                  onChange={(e) => updateExpField(i, "role", e.target.value)}
                />
                <Input
                  label="Location"
                  value={exp.location}
                  onChange={(e) =>
                    updateExpField(i, "location", e.target.value)
                  }
                />
                ...
                <Input
                  ...
                  value={exp.startDate?.slice(0, 10)}
                  onChange={(e) =>
                    updateExpField(i, "startDate", e.target.value)
                  }
                />
              ...
              <textarea
                ...
                value={exp.description}
                onChange={(e) =>
                  updateExpField(i, "description", e.target.value)
                }
              />
